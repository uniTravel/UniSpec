...
module LineParser =
    ...
    let (|BulletLine|_|) s =
        tryRegex s "^\s*\*\s(.*)"
        |> Option.map (function Trim t -> BulletLine t)

    let (|ScenarioLine|_|) (s: string) =
        let trimmed = s.Trim()
        seq { "Scenario"; "Example" }
        |> Seq.tryPick (fun x ->
            if trimmed |> startsWith x then ScenarioLine trimmed |> Some
            else None)
    let (|OutlineLine|_|) (s: string) =
        let trimmed = s.Trim()
        seq { "Scenario Outline"; "Scenario Template" }
        |> Seq.tryPick (fun x ->
            if trimmed |> startsWith x then OutlineLine trimmed |> Some
            else None)
    let (|ExamplesLine|_|) (s: string) =
        if s.Trim() |> startsWith "Examples" then Some ExamplesLine else None
    let (|RuleLine|_|) (s: string) =
        let trimmed = s.Trim()
        if trimmed |> startsWith "Rule" then RuleLine trimmed |> Some
        else None

    let (|Attributes|_|) (s: string) =
        if s.Trim() |> startsWith "@" then
            let tags = [ for tag in Regex.Matches(s, @"@(\w+)") do yield tag.Value.Substring(1) ]
    ...
    let (|BackgroundLine|_|) (s: string) =
        if s.Trim() |> startsWith "Background" then Some BackgroundLine else None
    ...
    let parse = function
        ...
        | Item (Examples, TableRow _), OutlineLine text
            -> Outline text
        ...
        | Item (l, TableRow _), TableRowLine row
            -> Item (l, TableRow row)
        | (Step _ as l), DocMarker _
        | (OutlineStep _ as l), DocMarker _
            -> Item (l, MultiLineStart)
        | Item (l, MultiLineStart _), Trim line
        | Item (l, MultiLine _), Trim line
