...
module BlockParser =
    ...
    let parseTable lines =
        let rec readTableRows rows lines =
            match lines with
            | (_, _, Item (_, TableRow row)) :: xs ->
                readTableRows (row :: rows) xs
    ...
    let parse (lines: string seq) =
        ...
        |> Seq.map (fun (number, content) ->
            ...
            if i = -1 then number, content
            else number, content.Substring(0, i))
        |> Seq.filter (fun (_, content) -> content.Trim().Length > 0)
        |> Seq.scan (fun (_, _, lastParsedLine) (number, content) ->
            let parsed = LineParser.parse (lastParsedLine, content)
            match parsed with
            | SyntaxError -> failwithf "Syntax error on line %d %s\r\n%s" number content <| LineParser.expecting lastParsedLine
            | Tag _ -> number, content, lastParsedLine
            | _ -> number, content, parsed
            ) (0, "", FileStart)
        |> Seq.filter (fun (_, _, line) ->
            match line with
            | FeatureDescription _ | Description _ | Rule _ -> false
            | _ -> true )
        |> Seq.toList
        |> parseFeatureFile